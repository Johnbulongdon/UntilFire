...
both Claude and Codex. Read the roadmap, recent changelog and commits, then
inspect relevant code before editing. Use available skills and slash commands
...
The app repo owns engineering and current design contracts. The Obsidian vault
owns strategy, research, feedback, and rationale. See the shared rules for the
verified vault branch mismatch and reconciliation follow-up. Save meaningful
engineering decisions with the change rather than only in chat.
