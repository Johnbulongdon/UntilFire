...
export const learnArticles: LearnArticle[] = [
  ...
  {
    slug: 'monte-carlo-simulation-fire-planning',
    title: 'Monte Carlo Simulation: A Better Way to Test Your FIRE Plan',
    description: 'Why running thousands of random market scenarios gives a more honest picture of retirement success than a single average-return projection.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '6 min read',
    body: [
      h2('The problem with single-scenario projections'),
      p('The standard retirement projection shows a single line: your portfolio growing at 7% per year until a target date, then declining as you withdraw. It is clean, intuitive, and deeply misleading.'),
      p('The problem: markets do not return 7% every year. They return -30% some years and +25% others. The order of those returns matters enormously, especially in the years around retirement when the portfolio is largest. A single average-return line hides the entire risk landscape.'),
      h2('What Monte Carlo simulation does differently'),
      p('Monte Carlo simulation runs thousands of randomised scenarios — each one using a different sequence of annual returns drawn from a realistic distribution. Instead of one line, you get a cloud of possible futures. Some lines end with a massive portfolio; others deplete to zero. The simulation counts what percentage of paths survive 30, 40, or 50 years at your withdrawal rate.'),
      p('That survival percentage is your success rate. A 90% success rate means 900 out of 1,000 random scenarios still had money left at the end of your retirement horizon.'),
      h2('How to interpret success rates'),
      p('Success rate thresholds used by most practitioners:'),
      ul([
        '95%+: very safe. Appropriate if you have no flexibility whatsoever (fixed costs, no supplemental income).',
        '85–95%: safe for most people. The 15% failure scenarios typically involve extreme, prolonged bear markets — rare but possible.',
        '75–85%: acceptable if you have spending flexibility or supplemental income. A modest cut in bad years dramatically improves outcomes.',
        'Below 75%: too risky for most early retirees. The plan needs more capital, lower spending, or supplemental income.',
      ]),
      p('Important caveat: Monte Carlo "failure" does not necessarily mean bankruptcy. Many simulations define failure as the portfolio reaching zero — but a real retiree would adjust spending well before that. A 90% success rate with spending flexibility is more secure than a 95% rate with rigid fixed withdrawals.'),
      h2('What inputs drive Monte Carlo results'),
      p('The key inputs to any Monte Carlo model:'),
      ul([
        'Expected return: usually calibrated to historical equity and bond returns (7% nominal stocks, 2–3% nominal bonds).',
        'Volatility (standard deviation): how much returns vary year to year. Higher volatility = wider range of outcomes.',
        'Inflation rate: typically 2–3% annually for US dollar portfolios.',
        'Asset allocation: a 100% equity portfolio has higher expected return but also higher volatility, producing more extreme outcomes in both directions.',
        'Withdrawal amount and strategy: fixed withdrawal vs dynamic (reduce in bad years) changes outcomes dramatically.',
      ]),
      h2('Historical sequence vs Monte Carlo: which is better?'),
      p('Some planners prefer historical sequence analysis — testing against every actual 30-year period in US market history. This is also called "FIRECalc"-style analysis.'),
      p('Historical analysis has the advantage of using real data, including the actual correlations between stocks, bonds, and inflation. Its weakness: it only covers ~100 years of US market data, limiting the number of independent 30-year periods to test.'),
      p('Monte Carlo can generate thousands of scenarios but relies on statistical assumptions about return distributions that may not hold in all environments. Best practice: use both. If your plan survives a 90%+ Monte Carlo AND all historical periods, you have strong confidence.'),
      h2('Practical tools'),
      p('Free Monte Carlo tools accessible to FIRE planners:'),
      ul([
        'FIRECalc: tests against all historical US market periods.',
        'Portfolio Visualizer: robust Monte Carlo with customisable asset classes and withdrawal strategies.',
        'cFIREsim: community-developed FIRE-specific calculator with inflation and Social Security inputs.',
        'UntilFire dashboard: run projections against your actual numbers.',
      ]),
      p('Run your plan at least annually, and whenever a major life event changes your spending or income assumptions. A Monte Carlo result from 5 years ago is not current.'),
    ],
  },
  {
    slug: 'fi-number-vs-retire-early-decision',
    title: 'FI Number vs the Retire Early Decision: They Are Not the Same',
    description: 'Why reaching your financial independence number and deciding to stop working are two separate choices — and why many people who hit FI keep working anyway.',
    category: 'FIRE Basics',
    publishedAt: '2026-06-18',
    readTime: '5 min read',
    body: [
      h2('The conflation at the heart of FIRE'),
      p('FIRE is often talked about as a single destination: reach the number, stop working. But practitioners who have actually reached financial independence describe it differently. The FI number and the retire-early decision are two distinct events — and for many people, they do not happen simultaneously.'),
      p('The FI number is a mathematical calculation: your investable portfolio equals 25× your annual spending. That is a financial fact. The retire-early decision is a life choice with psychological, social, and identity dimensions the calculator does not touch.'),
      h2('What financial independence actually gives you'),
      p('Reaching FI does not force you to stop working. What it does is remove the financial pressure from the decision. You work because you want to, not because the mortgage requires it.'),
      p('This shift — from "I have to work" to "I choose to work" — is reported by most FI-achievers as the most significant change. The work often improves because the desperation is gone. Negotiating power increases. The ability to walk away from a bad situation is real, not theoretical.'),
      h2('The one-more-year syndrome'),
      p('One of the most documented psychological patterns in the FIRE community is "one more year syndrome" — continuing to work past your FI number because the number feels uncertain, or because leaving feels irreversible, or because identity is tied to the career.'),
      p('The syndrome is rational in some ways: an extra year of work adds one year of contributions and avoids one year of withdrawals. For a $1,500,000 portfolio with $75,000 annual savings, one more year might add $150,000–$200,000 to the final balance, meaningfully improving safety margins.'),
      p('But it can also become a trap. When every year feels like "just one more year," FI never translates into RE. The accumulation phase becomes permanent.'),
      h2('FI as a spectrum, not a binary'),
      p('Many FIRE practitioners think of FI as a dial rather than a switch:'),
      ul([
        'Partial FI (50–75% of FIRE number): enough to take a career break, go part-time, or switch to lower-paying meaningful work without financial anxiety.',
        'Coast FI: enough invested that contributions can stop and the portfolio will grow to full FI by traditional retirement age on its own.',
        'Full FI (100% of FIRE number): the portfolio covers all spending indefinitely at your chosen withdrawal rate.',
        'Fat FI (125–150%+): substantial buffer above the baseline, supporting lifestyle upgrades or the ability to weather extended bad markets without adjustment.',
      ]),
      p('Many people take meaningful action at 75% FI even though they have not "hit the number." They negotiate part-time, move to a less stressful role, or launch a project they have been deferring. The number enables options well before it enables full retirement.'),
      h2('Designing the RE decision deliberately'),
      p('The retire-early decision is worth planning as carefully as the FI number. Questions worth answering before leaving work:'),
      ul([
        'What will I do with my time? Unstructured time is harder than expected for high-achieving people accustomed to external structure.',
        'What does my social life look like? Work provides social contact that disappears suddenly at retirement.',
        'What is my identity outside of my career? "I am a software engineer" stops applying on day one of retirement.',
        'What will I do if I get bored or miss the structure? A plan for re-engagement (consulting, part-time, projects) removes the feeling that retirement is irreversible.',
      ]),
      p('The FI number is the easy part. The harder work is answering what you are moving toward, not just what you are leaving.'),
      h2('The value of testing retirement before committing'),
      p('Many FIRE practitioners recommend a sabbatical — 3–6 months away from work — before making the final decision. The sabbatical tests whether you enjoy unstructured time, reveals what activities actually fill the day, and removes the romanticised image of retirement that exists only in theory.'),
      p('Some people return from the sabbatical ready to retire permanently. Others return to work with renewed energy and a clearer picture of how much work actually matters to them. Either outcome is better than a poorly considered permanent decision.'),
    ],
  },
  {
    slug: 'side-income-and-fire-how-small-income-shrinks-your-number',
    title: 'Side Income and FIRE: How a Small Income Stream Shrinks Your Number',
    description: 'How even modest side income — $10,000–$20,000 per year from consulting, freelance, or a passion project — can shorten your FIRE timeline or dramatically reduce your required portfolio.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '5 min read',
    body: [
      h2('The disproportionate leverage of a small income'),
      p('The 4% rule calculates your FIRE number based on total annual spending. Reduce that spending — or cover part of it with income — and the required portfolio drops dramatically.'),
      p('A $60,000/year spender needs $1,500,000. If that same person earns $15,000/year from freelance work, consulting, or a creative project, the portfolio only needs to cover $45,000. FIRE number: $1,125,000. That $15,000/year in side income reduced the required portfolio by $375,000.'),
      p('At a $75,000/year savings rate, $375,000 represents five fewer years of work. A small income stream can shorten the path to FIRE by years before you ever stop working.'),
      h2('Why side income is more valuable in retirement than accumulation'),
      p('Side income during the accumulation phase increases your savings rate (already covered by savings rate math). But side income in the early years of retirement does something more powerful: it eliminates or reduces portfolio withdrawals during the most dangerous sequence-of-returns window.'),
      p('If you retire at 45 and earn $15,000/year for the first 5 years through occasional consulting, those 5 years of reduced withdrawals can extend your portfolio\'s survival by a decade. The portfolio avoids selling shares during what may be its most volatile years.'),
      h2('Barista FIRE: the structured version'),
      p('Barista FIRE is the formalised version of this strategy. You reach partial financial independence — enough that a small income covers the gap between investment income and spending — then work a low-stress, low-hour job. The original name came from the observation that a Starbucks barista benefits include healthcare, which early retirees often need at low cost.'),
      p('Barista FIRE makes sense when:'),
      ul([
        'You are 75–80% of the way to your full FIRE number and the rest will take years you do not want to spend.',
        'You have marketable skills that are easy to deploy part-time without significant stress.',
        'Healthcare coverage from part-time work eliminates the ACA marketplace cost.',
        'The work is low enough stress that it does not recreate the burnout you were escaping.',
      ]),
      h2('Types of side income compatible with FIRE'),
      p('The ideal FIRE-compatible income source is:'),
      ul([
        'Flexible hours — you control when and how much you work',
        'Low fixed commitment — no full-time salary means no full-time obligation',
        'Skill-based — leverages what you already know without relearning everything',
        'Location-independent — works from anywhere, compatible with geographic arbitrage',
      ]),
      p('Examples that fit these criteria: freelance writing, consulting in your professional area, online teaching or courses, software contracting, photography or creative work, rental income, and occasional high-value project work. None of these require 40 hours per week.'),
      h2('The income floor concept'),
      p('Some FIRE planners set an "income floor" goal rather than a specific retirement date. The target: build enough passive and side income to cover essential expenses ($2,000–$3,000/month) regardless of portfolio performance. With that floor in place, portfolio volatility is less threatening — you never have to sell stocks in a downturn if the income floor covers basics.'),
      p('The income floor can come from multiple small streams: $800/month rental income, $600/month freelance, $400/month dividend yield, $300/month part-time work. Together they provide stability without any single source being indispensable.'),
      h2('The productivity dividend'),
      p('Beyond portfolio math, side income in early retirement provides a psychological benefit: purpose, structure, and social contact that retirement alone does not provide. The most satisfied early retirees tend to have some productive engagement — not 40-hour weeks, but meaningful work that uses their skills and earns something.'),
      p('The financial benefit and the psychological benefit point in the same direction. Small income in early retirement is not a compromise on FIRE ideals — it is often the most robust version of the plan.'),
    ],
  },
]
...
const articleMetaBySlug: Record<string, LearnArticleMeta> = {
  ...
  'monte-carlo-simulation-fire-planning': {
    primaryStage: 'approaching-fire',
    secondaryStages: ['building-momentum', 'living-in-fire'],
    relatedCalculators: [
      { href: '/calculators/4-percent-rule', label: 'Safe Withdrawal Calculator' },
      { href: '/dashboard', label: 'Dashboard Projections' },
    ],
  },
  'fi-number-vs-retire-early-decision': {
    primaryStage: 'approaching-fire',
    secondaryStages: ['building-momentum'],
    relatedCalculators: [
      { href: '/calculators/4-percent-rule', label: 'FIRE Number Calculator' },
      { href: '/?source=learn-fi-vs-re', label: 'FIRE Calculator' },
    ],
  },
  'side-income-and-fire-how-small-income-shrinks-your-number': {
    primaryStage: 'building-momentum',
    secondaryStages: ['approaching-fire', 'living-in-fire'],
    relatedCalculators: [
      { href: '/calculators/savings-rate', label: 'Savings Rate Calculator' },
