...
export const learnArticles: LearnArticle[] = [
  ...
  {
    slug: 'safe-withdrawal-rate-3-4-5-percent-debate',
    title: 'The Safe Withdrawal Rate Debate: 3%, 4%, or 5%?',
    description: 'Why financial researchers disagree on the right withdrawal rate, and how to choose between 3%, 4%, and 5% for your own FIRE plan.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '7 min read',
    body: [
      h2('Why there is no single right answer'),
      p('The 4% rule is the most cited withdrawal rate in FIRE — but it is not a law. It is a historical observation based on one dataset (US markets 1926–1995) over one time horizon (30 years). Researchers and practitioners debate whether 3%, 4%, or even 5% is the right floor for modern FIRE planners.'),
      p('The debate matters because the number you choose defines your portfolio target. A $60,000 annual spend requires $1,500,000 at 4%, $2,000,000 at 3%, and $1,200,000 at 5%. That difference changes how long you need to save by years.'),
      h2('The case for 4%: historical durability'),
      p('The original Trinity Study found that a 60/40 portfolio survived 95% of 30-year windows at a 4% withdrawal rate. Subsequent research by Wade Pfau, Kitces, and others has largely confirmed the finding for traditional 30-year retirements.'),
      p('For someone retiring at 60–65 with a 30-year horizon and a willingness to cut spending modestly in down years, 4% holds up well historically.'),
      h2('The case for 3%: longer horizons and lower valuations'),
      p('Three arguments push FIRE planners toward 3%:'),
      ul([
        'Early retirees face 50-60 year horizons. The original 30-year calibration simply does not cover this range reliably.',
        'Current equity valuations (measured by Shiller CAPE ratio) are elevated relative to history. High starting valuations correlate with lower subsequent 10–15 year returns.',
        'International historical data shows lower safe withdrawal rates than US-only data — typically 3–3.5% — because the US had unusually strong 20th-century returns.',
      ]),
      p('Researchers like Wade Pfau have published analysis suggesting that for 40–50 year retirements, a 3.5% or lower withdrawal rate is more reliable than 4% when using globally diversified portfolios.'),
      h2('The case for 5%: flexibility and supplemental income'),
      p('Some practitioners argue that strict portfolio-based rules ignore real-world flexibility:'),
      ul([
        'Most retirees are not robotic: they cut spending in bad years, pick up part-time work, or adjust lifestyle.',
        'Social Security, rental income, or a small side income effectively reduce the portfolio withdrawal needed.',
        'Dynamic withdrawal strategies (withdrawing less when the portfolio is down) dramatically improve survival rates at higher percentages.',
      ]),
      p('Bill Bengen, the originator of the 4% rule, later updated his research to suggest that with the right asset allocation, 4.5–5% may be achievable — especially if the retiree has flexibility in spending.'),
      h2('How to choose: a practical framework'),
      p('Rather than picking one number dogmatically, use this decision tree:'),
      ol([
        'Start at 4%: it is historically robust for 30 years and widely validated.',
        'Drop to 3.5% if: you are retiring before 55, or current valuations are high (Shiller CAPE > 30), or your plan relies entirely on the portfolio with no flexibility.',
        'Drop to 3% if: you are retiring before 45, you want maximum safety margin, or you have high fixed costs with little ability to cut spending.',
        'Consider 4.5% if: you have significant supplemental income (Social Security, rental, part-time work) that will kick in within 10 years, reducing portfolio dependency.',
      ]),
      h2('The most robust approach: build in flexibility'),
      p('Research consistently shows that dynamic withdrawal strategies outperform fixed rules. The simple version: if your portfolio drops more than 15% in a year, withdraw 10% less than planned. If your portfolio grows substantially, allow yourself modest lifestyle increases. This flexibility alone can extend a portfolio from 30 to 50+ years at the same starting withdrawal rate.'),
      p('The withdrawal rate debate is ultimately about the cost of uncertainty. A 3% rate gives you more buffer; a 4% rate assumes reasonable flexibility. The worst outcome is targeting 5% on a fixed spending plan with no room to adjust — that is the scenario most vulnerable to a bad sequence of early returns.'),
    ],
  },
  {
    slug: 'net-worth-tracking-why-and-how',
    title: 'Net Worth Tracking: Why and How to Measure It Every Month',
    description: 'Why net worth is the single most important number to track on the path to FIRE, and the simplest method to update it monthly without spreadsheet fatigue.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '5 min read',
    body: [
      h2('Why net worth is the right metric'),
      p('Income tells you what you earn. Savings rate tells you how efficiently you earn. But net worth is the scoreboard — the accumulated result of every financial decision you have ever made. On the path to FIRE, net worth is the direct proxy for how far you are from financial independence.'),
      p('If your FIRE number is $1,250,000, your net worth tells you exactly where you stand. $400,000 means you are 32% of the way there. $800,000 means you are 64%. No other single number gives you this view.'),
      h2('What counts as net worth'),
      p('Net worth = total assets minus total liabilities.'),
      p('Assets to include:'),
      ul([
        '401(k), Roth IRA, and other investment accounts (at current market value)',
        'Taxable brokerage accounts',
        'Cash and high-yield savings accounts',
        'Home equity (market value minus remaining mortgage, if applicable)',
        'Any real estate equity',
      ]),
      p('Assets to exclude from your FIRE calculation (or treat as bonus, not foundation):'),
      ul([
        'Car value — depreciates and is likely to be replaced',
        'Primary home equity — unless you plan to downsize or relocate in retirement',
        'Personal property, jewelry, collectibles',
      ]),
      p('Liabilities to include: mortgage balance, student loans, car loans, credit card balances, and any other debt.'),
      h2('How often to update net worth'),
      p('Once per month is the right cadence. More frequent is not useful — markets move daily, and checking weekly causes anxiety without insight. Less frequent means you lose the feedback loop between decisions and outcomes.'),
      p('A good time to update: the first weekend of each month. Set a 20-minute calendar block.'),
      h2('The simplest tracking method'),
      p('A spreadsheet with one row per month works for most people. The columns you need:'),
      ul([
        'Date',
        'Investment accounts total (pull from each account login)',
        'Cash and savings total',
        'Home equity (optional, update quarterly if you own)',
        'Debts total (pull from lender portals)',
        'Net worth = assets minus debts',
        'Month-over-month change',
        'FIRE progress (net worth / FIRE number as a percentage)',
      ]),
      p('That last column — FIRE progress — is the most motivating number in the whole spreadsheet. Watching it inch from 20% to 25% to 30% makes the long timeline feel real.'),
      h2('What to do when net worth drops'),
      p('Market downturns will cause your investment accounts to drop, shrinking net worth. This is expected and not a signal to change anything. The value of tracking is that it keeps you calm: you can see the history and recognise that a 15% drop is normal, not catastrophic.'),
      p('The useful response to a net worth drop: check that your savings rate has not changed (spending creep is the silent killer), rebalance if allocation has drifted, and otherwise hold.'),
      h2('Net worth vs liquid investable net worth'),
      p('Your FIRE number depends on investable assets — what you could actually withdraw from in retirement. Home equity does not count unless you plan to sell. A car worth $15,000 does not count either.'),
      p('Track both: total net worth as the big picture, and investable net worth as your FIRE progress metric. The gap between them tells you how much is tied up in non-financial assets.'),
    ],
  },
  {
    slug: 'debt-payoff-vs-investing-which-comes-first',
    title: 'Debt Payoff vs. Investing: Which Comes First?',
    description: 'The interest rate threshold framework for deciding whether to pay down debt or invest — and the role of psychology when the math is ambiguous.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '6 min read',
    body: [
      h2('The core question: guaranteed return vs expected return'),
      p('Paying off debt gives you a guaranteed return equal to the interest rate. If your student loan charges 6%, every dollar you pay down earns you a guaranteed 6% return — no market risk.'),
      p('Investing in a stock market index fund historically returns 7–10% per year in nominal terms. But that return is not guaranteed — any given year might be -30% or +30%. The expected long-run average is positive, but the sequence matters enormously.'),
      p('The basic framework: if the debt interest rate is higher than expected after-tax investment returns, pay off debt first. If lower, invest first. If roughly equal, it is a toss-up and psychology becomes the deciding factor.'),
      h2('The interest rate thresholds'),
      p('Most FIRE practitioners use these rough thresholds:'),
      ul([
        'Above 7%: pay off first. High-interest debt (most credit cards charge 18–24%) destroys wealth faster than investing builds it.',
        '4–7%: this is the gray zone. Math favors investing slightly, but the guaranteed return of debt payoff is valuable too.',
        'Below 4%: invest first. Mortgage debt, subsidised student loans, and car loans at 2–3% are cheap enough that investing in the market beats paying them down.',
      ]),
      p('Credit card debt at 20%+ is always the highest-priority item on any balance sheet. No investment reliably beats 20% guaranteed returns.'),
      h2('The tax adjustment'),
      p('Investment returns are taxed. A 7% market return might net to 5.5% after long-term capital gains tax in a taxable account. Meanwhile, mortgage interest is deductible for some households, effectively lowering the interest rate.'),
      p('For a more precise comparison, use after-tax debt rate versus after-tax investment return. In a Roth IRA or 401(k), returns grow tax-free, which makes the case for investing stronger against moderate-rate debt.'),
      h2('Why employer match changes everything'),
      p('If your employer offers a 401(k) match, that is a 50–100% instant return on your contribution. No debt repayment strategy beats that. Always contribute enough to capture the full employer match before accelerating debt payoff — the match is free money.'),
      h2('The psychological argument for debt payoff'),
      p('The math sometimes points to investing. But debt has a weight that numbers do not capture. For many people, carrying debt creates anxiety that reduces financial decision quality. If debt causes:'),
      ul([
        'Chronic stress that affects work performance or relationships',
        'Difficulty sleeping or thinking about money clearly',
        'A reluctance to invest at all because the debt feels unresolved',
      ]),
      p('Then paying off debt first is the right move — not because the math says so, but because it unlocks the mental capacity to invest aggressively afterward. A debt-free person with a 40% savings rate will beat a debt-laden person investing 10% because they cannot relax.'),
      h2('The optimal FIRE sequence'),
      p('For most people on the FIRE path:'),
      ol([
        'Build a small emergency fund ($1,000–$2,000) to prevent new debt.',
        'Contribute to 401(k) up to the employer match — always.',
        'Pay off all high-interest debt (> 7%) aggressively.',
        'Max Roth IRA and 401(k) contributions.',
        'Invest in a taxable brokerage account.',
        'If moderate debt (4–7%) remains, split extra cash 50/50 between debt payoff and taxable investing.',
        'Once debt is gone, redirect the full payment toward investing.',
      ]),
      p('The critical insight: in most cases, this sequence means reaching FIRE later than purely investing would — but doing so with less fragility. Carrying a mortgage into early retirement is fine; carrying high-interest debt is not.'),
    ],
  },
]
...
const articleMetaBySlug: Record<string, LearnArticleMeta> = {
  ...
  'safe-withdrawal-rate-3-4-5-percent-debate': {
    primaryStage: 'approaching-fire',
    secondaryStages: ['living-in-fire'],
    relatedCalculators: [
      { href: '/calculators/4-percent-rule', label: 'Safe Withdrawal Calculator' },
  ...
  'net-worth-tracking-why-and-how': {
    primaryStage: 'building-momentum',
    secondaryStages: ['starting-out', 'approaching-fire'],
    relatedCalculators: [
      { href: '/?source=learn-networth', label: 'FIRE Calculator' },
      { href: '/calculators/savings-rate', label: 'Savings Rate Calculator' },
    ],
  },
