...
export const learnArticles: LearnArticle[] = [
  ...
  {
    slug: 'rebalancing-bands-vs-calendar-rebalancing',
    title: 'Rebalancing Bands vs Calendar Rebalancing: Which Approach Works Better?',
    description: 'How threshold-based rebalancing compares to annual calendar rebalancing — and when each approach makes sense for a FIRE portfolio.',
    category: 'Investing',
    publishedAt: '2026-06-18',
    readTime: '6 min read',
    body: [
      h2('Two approaches to rebalancing'),
      p('Once you have a target asset allocation — say 80% stocks and 20% bonds — the question is when to bring it back into alignment after markets move it off target. Two approaches dominate: calendar rebalancing (rebalance on a fixed schedule, typically annually) and band rebalancing (rebalance whenever any asset class drifts beyond a threshold).'),
      p('Both approaches work. The debate between them is about frequency, tax cost, and effort — not about whether rebalancing itself is valuable. The benefits of rebalancing are clear: it enforces the discipline of selling high and buying low, and it keeps your risk profile consistent with your intentions.'),
      h2('Calendar rebalancing: the simplest approach'),
      p('Calendar rebalancing means picking a date — often January or your tax filing date — and rebalancing to target regardless of how far you have drifted. This approach has several advantages:'),
      ul([
        'Zero monitoring required. You check once a year and act.',
        'Predictable: you know exactly when you will rebalance, making it easier to combine with tax-loss harvesting and contribution decisions.',
        'Low transaction costs: one rebalancing event per year minimises trading.',
      ]),
      p('The main drawback is that markets can drift significantly in a single year. A 20% equity rally can push an 80/20 portfolio to 85/15 or beyond before the annual rebalancing date arrives. For a long-term investor, this drift is generally tolerable.'),
      h2('Band rebalancing: the 5/25 rule'),
      p('Band rebalancing triggers when any asset class drifts beyond a threshold. The most cited framework is the 5/25 rule, popularised by financial planner Larry Swedroe:'),
      ul([
        'Absolute band: rebalance if any asset class moves more than 5 percentage points from target (e.g., bonds drift from 20% to 25% or 15%).',
        'Relative band: rebalance if any asset class moves more than 25% of its target allocation (e.g., bonds at 20% target — rebalance if they reach 25% or 15%).',
        'Use whichever threshold is reached first.',
      ]),
      p('Band rebalancing responds to market conditions rather than the calendar. In volatile years, it may trigger multiple rebalancing events. In calm years, it may trigger none. Research suggests it slightly outperforms calendar rebalancing on risk-adjusted returns, but the difference is small in practice.'),
      h2('Tax implications of each approach'),
      p('In taxable accounts, every rebalancing event that involves selling appreciated assets triggers capital gains. Calendar rebalancing in a taxable account typically produces one capital gains event per year. Band rebalancing could produce more, adding tax drag.'),
      p('In tax-advantaged accounts (401k, Roth IRA), both approaches are tax-free — so band rebalancing is more attractive there. A practical hybrid: use bands in tax-advantaged accounts where there is no transaction cost, and calendar rebalancing in taxable accounts to control the tax timing.'),
      h2('A FIRE-practical approach'),
      p('For most FIRE investors, the practical answer is to combine both: check quarterly, rebalance only if bands are breached (5% absolute), and do a full calendar review every December regardless. This catches major drifts without creating excessive trading, while ensuring at least one intentional review per year.'),
      p('The most important rebalancing decision is not bands vs calendar — it is actually doing it. Either method consistently applied produces far better outcomes than haphazard or emotion-driven allocation changes.'),
      h2('Using contributions to rebalance'),
      p('During accumulation, the simplest rebalancing method is contribution-based: direct new investments toward whichever asset class is currently underweight. This rebalances without selling anything, which eliminates tax consequences in taxable accounts entirely. A $2,000 monthly contribution can do significant rebalancing work without triggering a single capital gain.'),
    ],
  },
  {
    slug: 'fire-number-sensitivity-analysis-what-changes-your-timeline',
    title: 'FIRE Number Sensitivity: Which Assumptions Matter Most for Your Timeline',
    description: 'How small changes to spending, return assumptions, withdrawal rate, and inflation cascade through FIRE math — and which variables give you the most control over your retirement date.',
    category: 'Planning',
    publishedAt: '2026-06-18',
    readTime: '7 min read',
    body: [
      h2('Your FIRE number is not a single point'),
      p('Most FIRE calculators produce a single number: your FIRE date and required portfolio size. This precision is useful but misleading. The output is extremely sensitive to input assumptions, and small changes in those assumptions produce dramatically different timelines. Treating the output as a point estimate rather than a range leads to overconfidence.'),
      p('Sensitivity analysis replaces the single estimate with a range: what does the FIRE date look like if returns are 5% instead of 7%? If spending is $5,000 higher? If you retire at 3.5% instead of 4%? Running these scenarios takes 20 minutes and changes how you think about the plan.'),
      h2('The five most sensitive assumptions'),
      h3('1. Annual spending'),
      p('Every $1,000 per month in lower spending removes $300,000 from your required portfolio (at 25x) and reduces the timeline in two ways: the target is smaller and you are saving more of each paycheque. Spending is the single most powerful lever in FIRE math. A household cutting $500/month ($6,000/year) from their budget moves their FIRE date by 1–3 years depending on timeline.'),
      h3('2. Investment return assumption'),
      p('The difference between assuming 6% and 8% real returns (after inflation) changes a 20-year timeline by 2–4 years for most portfolios. Conservative planners use 5–6% real; historical US equity returns average about 7% real. Assuming 10% or higher is optimistic and dangerous. For a 50-year retirement horizon, planning for 6% real is more prudent than the historical average.'),
      h3('3. Withdrawal rate'),
      p('Moving from a 4% to a 3.5% withdrawal rate increases the required portfolio by 14% (25x becomes 28.6x). For a household targeting $60,000/year spending, this adds $214,000 to the required portfolio — roughly 2–4 additional years of saving. For 50-year retirements, 3.5% is generally safer; for 30-year retirements, 4% has strong historical support.'),
      h3('4. Inflation assumption'),
      p('FIRE calculations usually use real returns (already adjusted for inflation). But when estimating Social Security, pension income, or spending trajectories, inflation matters separately. Healthcare inflation has historically run 2–3% above general inflation — a significant risk for early retirees who face 30+ years before Medicare.'),
      h3('5. Current portfolio size and savings rate'),
      p('A household with $300,000 already saved and a $50,000/year savings rate reaches $1,250,000 in about 15 years at 7% returns. The same household with $100,000 saved takes 18 years. The starting portfolio is a 3-year swing — roughly equivalent to increasing the savings rate by $10,000/year. Both matter.'),
      h2('Running your own sensitivity table'),
      p('A simple sensitivity table tests three scenarios across two variables:'),
      ol([
        'Base case: 7% real return, 4% withdrawal rate, current spending.',
        'Conservative case: 5% real return, 3.5% withdrawal rate, spending 10% higher.',
        'Optimistic case: 8% real return, 4.5% withdrawal rate, spending 10% lower.',
      ]),
      p('The range of FIRE dates across these scenarios is your honest uncertainty range. If the conservative case is still acceptable (maybe 5 years later than the base case), you can proceed with confidence. If the conservative case is 15 years later, the plan is fragile and needs structural changes.'),
      h2('The variable you actually control'),
      p('Return assumptions are outside your control. Withdrawal rate is a planning decision. Spending is the variable you control most directly. The clearest message from sensitivity analysis is almost always: small, sustained reductions in spending have more impact on FIRE timeline than trying to chase higher returns or time the market. Optimise the inputs you control first.'),
    ],
  },
  {
    slug: 'one-more-year-syndrome-knowing-when-to-retire',
    title: 'One More Year Syndrome: Why FIRE Investors Struggle to Actually Stop Working',
    description: 'Why financially independent people delay retirement longer than planned — the psychology behind one more year syndrome, and practical ways to recognize and address it.',
    category: 'Psychology',
    publishedAt: '2026-06-18',
    readTime: '6 min read',
    body: [
      h2('What is one more year syndrome?'),
      p('One more year syndrome (OMYS) is the pattern where someone who has reached their FIRE number — or is very close — finds a reason to keep working for "just one more year." Each year produces a new reason: the portfolio needs more buffer, a promotion is coming, the market looks uncertain, the kids are almost through college. The retirement date perpetually recedes.'),
      p('This is not a pathology. It reflects real psychological dynamics that the FIRE movement often underestimates: identity, purpose, fear of the unknown, loss aversion, and the discomfort of being "done." Understanding these dynamics is part of building a plan that actually gets executed.'),
      h2('The financial rationalisation'),
      p('The most common OMYS trigger is financial: "one more year of saving would make the portfolio 8% larger and significantly reduce sequence risk." This is mathematically true but often not the real concern. The portfolio is usually already large enough — the emotional need is for certainty that does not actually exist.'),
      p('No additional year of saving eliminates sequence risk or market uncertainty. A portfolio large enough to survive a 30% crash in year one does not become meaningfully safer by being 8% larger through another year of work. At some point the portfolio is adequate and additional working years represent a real cost in life time, not additional meaningful safety.'),
      h2('The identity problem'),
      p('Many high-achieving FIRE pursuers derive significant identity from their career. Work provides structure, social connection, intellectual challenge, and a clear sense of purpose. Retirement requires replacing all of these — and the FIRE community often underinvests in planning for this transition. The prospect of an unstructured life is genuinely frightening to people who have been intensely goal-oriented for decades.'),
      p('The symptom is financial rationalisation of what is actually an identity question: "I need more money" when the real concern is "I do not know who I am without this job."'),
      h2('Practical approaches to addressing OMYS'),
      ul([
        'Define retirement before reaching it. What does your typical Tuesday look like five years into retirement? If you cannot answer this, the discomfort about retirement is not about money.',
        'Set a hard stop date in advance. Announce it to your employer, spouse, and friends. External accountability reduces the "one more year" drift.',
        'Run a retirement test. Take a 4–6 week leave of absence and live as if retired. Most people discover they are either more comfortable than feared or identify specific gaps to address before the real transition.',
        'Build the life before leaving the job. Develop the social connections, projects, and routines you plan to have in retirement while still working. Transition into them; do not wait until day one of retirement to start.',
        'Accept asymmetric risk. The risk of retiring with enough money and running out is real but manageable — you can return to part-time work. The risk of dying having never used the freedom you saved for is unrecoverable.',
      ]),
      h2('When one more year is actually reasonable'),
      p('Not all delayed retirements are OMYS. Legitimate reasons to work one more year include: the portfolio is genuinely below the target, healthcare coverage will not otherwise be available, a specific pension vesting date is approaching, or a genuinely transformative career opportunity with a defined endpoint. The test is whether there is a concrete deliverable at the end of the year, or just a vaguer sense of not feeling ready.'),
      h2('The role of FI without RE'),
      p('Many people who reach FI — financial independence — choose not to RE — retire early. They continue working because they genuinely want to, not because they feel they must. This is a healthy outcome and a legitimate choice. The goal of FIRE is work optionality, not mandatory retirement. OMYS becomes a problem only when the "one more year" is driven by anxiety rather than genuine desire.'),
    ],
  },
]
...
const articleMetaBySlug: Record<string, LearnArticleMeta> = {
  ...
  'rebalancing-bands-vs-calendar-rebalancing': {
    ...
    relatedCalculators: [
      { href: '/calculators/compound-interest', label: 'Compound Interest Calculator' },
      { href: '/?source=learn-rebalancing-bands', label: 'FIRE Calculator' },
    ],
  },
  'fire-number-sensitivity-analysis-what-changes-your-timeline': {
    primaryStage: 'approaching-fire',
    secondaryStages: ['building-momentum'],
    relatedCalculators: [
      { href: '/calculators/4-percent-rule', label: 'FIRE Number Calculator' },
  ...
  'one-more-year-syndrome-knowing-when-to-retire': {
    primaryStage: 'approaching-fire',
    secondaryStages: ['living-in-fire'],
    relatedCalculators: [
      { href: '/calculators/4-percent-rule', label: 'FIRE Number Calculator' },
      { href: '/?source=learn-omys', label: 'FIRE Calculator' },
    ],
  },
}
