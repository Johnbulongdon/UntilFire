...
export default function GoalsScreen({ onNext, onBack }: { onNext: (goal: string) => void; onBack: () => void }) {
  ...
  return (
    <div className="uf-screen">
      ...
      <div className="uf-goals-grid">
        {FIRE_GOALS.map((g) => (
          <button key={g.id} className={`uf-goal-card ${selected === g.id ? "active" : ""}`} onClick={() => setSelected(g.id)}>
      ...
      <div className="uf-nav-row">
        ...
        <button className="uf-btn uf-btn-primary" style={{ flex: 1 }} disabled={!selected} onClick={() => selected && onNext(selected)}>
          Continue {"->"}
        </button>
      </div>
    </div>
