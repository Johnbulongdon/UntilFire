      ...
      <div className="uf-card uf-hero-split" style={{ padding: 0, overflow: "hidden", display: "flex", minHeight: 180 }}>
        ...
        <div className="uf-hero-left" style={{ flex: "0 0 55%", padding: "28px 32px", background: "#FFFFFF" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <div style={{ fontSize: 10, fontFamily: "Manrope, sans-serif", letterSpacing: "1.2px", textTransform: "uppercase", color: "#64748B", fontWeight: 700 }}>
              Your freedom date
            </div>
            {income > 0 && (
              <span style={{ fontSize: 10, fontWeight: 700, fontFamily: "Inter, sans-serif", color: statusColor, background: `${statusColor}20`, borderRadius: 20, padding: "2px 8px" }}>
                {statusLabel}
              </span>
            )}
          </div>
          {retireYear ? (
            <>
              ...
              {nextMoveScenarios?.[0] && (
                <div style={{ marginTop: 12, padding: "8px 12px", background: "rgba(5,150,105,0.06)", border: "1px solid rgba(5,150,105,0.15)", borderRadius: 8, fontSize: 12, color: "#047857", fontFamily: "Inter, sans-serif", fontWeight: 600 }}>
                  Top move: {nextMoveScenarios[0].label}
                </div>
              )}
            </>
          ) : (
      ...
      <div>
        ...
        {hasActuals ? (
          <>
            ...
            {(() => {
              const topCat = EXPENSE_CATS
                .map(c => ({ ...c, total: _actuals[c.key] || 0 }))
                .filter(c => c.total > 0)
                .sort((a, b) => b.total - a.total)[0];
              const cutScenario = nextMoveScenarios?.find(s =>
                s.label.toLowerCase().includes("cut") || s.label.toLowerCase().includes("expens")
              );
              if (!topCat || !cutScenario || cutScenario.deltaYears <= 0) return null;
              const accel = cutScenario.deltaYears >= 1
                ? `${cutScenario.deltaYears.toFixed(1)} years sooner`
                : `${Math.round(cutScenario.deltaYears * 12)} months sooner`;
              return (
                <div style={{ marginTop: 10, padding: "10px 14px", background: "rgba(249,115,22,0.05)", border: "1px solid rgba(249,115,22,0.15)", borderRadius: 10, fontSize: 13, color: "#9A3412", fontFamily: "Inter, sans-serif" }}>
                  {topCat.icon} Your biggest expense is <strong>{topCat.label}</strong> at {fmtMoney(topCat.total)}/mo — cutting it 10% would move your freedom date <strong>{accel}</strong>.
                </div>
              );
            })()}
          </>
