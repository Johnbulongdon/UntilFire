    ...
    >
      <div className="uf-share-modal">
        ...
        <div className="uf-share-card">
          ...
          <div className="uf-share-card-city" style={{ color: 'rgba(255,255,255,0.4)' }}>{years} years away Â· free calculator</div>
...
function RevealScreen({ city, income, savings, stateKey, fireGoal, currentAge, onAdjust }: {
  ...
  const calcLabels = ["City cost-of-living", "After-tax income", "Compound growth at 7%", "25Ã— withdrawal rule"];
          ...
          <div className="uf-calc-steps">
            {calcLabels.map((label, i) => (
              <span key={i} className={`uf-calc-step ${activeSteps.includes(i) ? "lit" : ""}`}>
                {label}
                {i < calcLabels.length - 1 && <span className="uf-calc-dot">Â·</span>}
              </span>
            ))}
          </div>
