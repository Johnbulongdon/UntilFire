...
type ShareCardKind = "identity" | "benchmark" | "date";
...
function ShareModal({
  cityName, fireIdentity, benchmark, freedomDate, isAlreadyFire, onClose, onShareCompleted,
}: {
  cityName: string;
  ...
  onClose: () => void;
  onShareCompleted?: (platform: 'x' | 'facebook' | 'reddit' | 'copy', card: ShareCardKind) => void;
}) {
  ...
  const freedomDateTitle = isAlreadyFire
    ? "I've reached FIRE 🔥"
    : `My freedom date: ${freedomDate ?? ""}`;
  const shareCards: Record<ShareCardKind, { label: string; title: string; body: string; text: string; privacy: string }> = {
    identity: {
      ...
      privacy: "No exact income · no FIRE number · no freedom date",
    },
    benchmark: {
      ...
      privacy: "No exact income · no FIRE number · no savings amount",
    },
    date: {
      label: "Card C · Freedom Date",
      title: freedomDateTitle,
      body: "Working toward work optionality with UntilFire.",
      text: `${freedomDateTitle}\nWorking toward work optionality. Find your freedom date → untilfire.com`,
      privacy: "No income · no FIRE number · no savings data shared",
    },
  };
  ...
  function copyToClipboard() {
    navigator.clipboard.writeText(`${activeShare.text}\n${shareUrl}`).then(() => {
      ...
      onShareCompleted?.("copy", selectedCard);
    });
  }
  ...
  function openShare(platform: "x" | "facebook" | "reddit") {
    ...
    onShareCompleted?.(platform, selectedCard);
  }
  ...
  return (
    ...
    >
      <div className="uf-share-modal">
        ...
        <div className="uf-share-subheading">Pick a card. No income, savings, or FIRE number is ever shared — just the insight.</div>
        ...
        <div className="uf-share-card">
          ...
          <div className="uf-share-card-label" style={{ textTransform: 'uppercase', letterSpacing: '2px', fontSize: 11 }}>{selectedCard === "identity" ? "FIRE Type" : selectedCard === "benchmark" ? "Benchmark" : "Freedom Date"}</div>
          ...
          <div className="uf-share-card-city" style={{ color: 'rgba(255,255,255,0.4)' }}>{activeShare.privacy}</div>
    ...
    <div className="uf-screen uf-reveal-screen">
      {showShare && (
        <ShareModal
          ...
          freedomDate={isAlreadyFire ? undefined : `${fireMonthFull} ${result.retireYear}`}
          isAlreadyFire={isAlreadyFire}
          onClose={() => setShowShare(false)}
          onShareCompleted={(platform, cardKind) => trackResultShareCompleted({ platform, cardKind })}
        />
      )}
          ...
          {revealed && (
            <div ref={belowRef}>
                    ...
                    <div className="uf-reveal-card" style={{ background: "#003527", color: "#fff", borderRadius: 16, paddingBottom: "clamp(18px, 2.5vw, 26px)", position: "relative", overflow: "hidden", boxShadow: "0 24px 40px -28px rgba(15,23,42,0.16)" }}>
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        <Link
                          ...
                          onClick={() => { saveCalculatorPrefill({ monthlyIncome: Math.round(takeHome / 12), monthlySavings: savings, monthlySpendEstimate: Math.max(0, Math.round(takeHome / 12 - savings)), cityName: city.name, stateKey, fireTarget: result.fireTarget, annualCost: city.col, retireYear: result.retireYear, generatedAt: new Date().toISOString(), currentAge, portfolioBalance, landingSource, defaultCurrency: currency }); trackResultSaveClicked({ placement: 'monthly_move', landingSource }); }}
                        >
                          Save my plan →
                        </Link>
                        <div style={{ marginTop: 8, fontSize: 11, color: "rgba(255,255,255,0.4)", textAlign: "center", lineHeight: 1.5 }}>
                ...
                <div style={{ background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: 16, padding: "clamp(16px,2.5vw,24px)", marginBottom: 10 }}>
                  ...
                  {!isAlreadyFire && (
                    <Link
                      ...
                      onClick={() => { saveCalculatorPrefill({ monthlyIncome: Math.round(takeHome / 12), monthlySavings: savings, monthlySpendEstimate: Math.max(0, Math.round(takeHome / 12 - savings)), cityName: city.name, stateKey, fireTarget: result.fireTarget, annualCost: city.col, retireYear: result.retireYear, generatedAt: new Date().toISOString(), currentAge, portfolioBalance, landingSource, defaultCurrency: currency }); trackResultSaveClicked({ placement: 'next_move', landingSource }); }}
                    >
                      Track my progress →
                    </Link>
                  )}
              ...
              <div data-gsap="footer-cta" style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 12, justifyContent: "center" }}>
                <button className="uf-btn-outline" onClick={() => { setShowShare(true); trackResultShareOpened({ landingSource }); }}>
                      ...
                      <form
                        onSubmit={async (e) => {
                          ...
                          try {
                            ...
                            if (res.ok) trackResultEmailCaptured({ landingSource });
