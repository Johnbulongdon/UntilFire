...
let states=[true,false,2],index=0;const exports={};
...
let tree=render();assert.equal(tree.props['data-motion'],'system');
const play=all(tree).find(n=>n.type==='button'&&n.props.children==='Play animations');assert.ok(play);play.props.onClick();
let intro=render();assert.equal(intro.props.reduce,false);assert.equal(intro.props.playMotion,true);intro.props.onDone();
tree=render();assert.equal(tree.props['data-motion'],'play');assert.ok(all(tree).find(n=>n.type==='button'&&n.props.children==='Replay reveal'));
console.log('Reduced-motion playback control passed: system default respected, explicit play enables introduction and replay.');
